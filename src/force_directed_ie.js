...
// d3.json("json/everything.json", function(json) {
	...
	function update(){
        ...
        var color = d3.scale.linear()
                                ...
                                // .domain([0, 3, 7, 11, 15, 19])
                                // .range(["#4288c2", "#68a1ce", "#90b9d9", "#b3cfe7", "#d9e7f2", "#ecf3f9"]);
                
        var famous_color = d3.scale.linear()
                        .domain([0, 6, 11, 28]) // 18])
                        .range(["#D94701", "#FD8D3C", "#FDBE85", "#FEEDDE"]);
                        // .domain([0, 3, 7, 11, 15, 19])
                        // .range(["#eb1d5d", "#ee4a7f", "#f4779f", "#f7a4be", "#fbd2e0", "#fde8ef"]);
        ...
        nodes = node_gs.append("circle")
         .attr("r", function(d){
             ...
             .style("fill", function(d) {
                        ...
                        if (d.famous || d.highlighted){
                            // return "blue";
                            return famous_color(d.distance);
                            // return famous_color(d.distance - min_distance);
                        }
                        else {
                            return color(d.distance);
	...
	function tick(e) {
		...
		// 	      d.target.y += k;
		// 	    });
        // The following line fixes the idiotic line problem in IE 10 and 11.
        // Solution from this StackOverflow thread: http://stackoverflow.com/a/18474904
		...
		json.nodes.forEach(function(d){
			// ancestors.forEach(function(d){
				if (d.isFixed){
					...
					d.y = h/3 + 2*d.generation*link_length;
			};
			// else {d.fixed = false};
		});
    ...
    // Previously, we stored the length of the name using this.getComputedTextLength(),
    // and now we'll use it to prevent the text from leaving the space.
    
    names_t.attr("x", function(d) {
                     var text_width = d.name_width;
